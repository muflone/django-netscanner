...
from netscanner.tools.zabbix_agent import ZabbixAgent


class Command(DiscoveryBaseCommand):
    help = 'Get information using zabbix-agent'
    tool_name = 'zabbix_agent'
    ...
    def instance_scanner_tool(self,
        ...
        :return:
        """
        return ZabbixAgent(verbosity=options.get('verbosity', 1),
                           timeout=discovery.timeout,
                           port=options.get('port', 10050),
                           items=options['items'])
